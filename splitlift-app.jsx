...
// ---------- ONBOARDING ----------
// 4 steps: Sport -> Body (h/w/age/sex) -> Training (days/cardio) -> Pick template.
...
function Onboarding({ onDone }) {
  ...
  const ranked = useMemo(() => (
    window.rankTemplatesForSport
      ? window.rankTemplatesForSport({ sport, days, limit: 3 })
      : []
  ), [sport, days]);

  // Default the picked template to the best fit on first arrival at step 3.
  useEffect(() => {
  ...
  const finish = () => {
    ...
    const tpl = (window.SPLIT_TEMPLATES || []).find(t => t.id === pickedTpl);
    let initialDays = window.INITIAL_DAYS;
    if (tpl && window.makeDayForType) {
      initialDays = tpl.days.map(t => window.makeDayForType(t, profile, null));
    }
    ...
    <div className="screen">
      ...
      <div className="screen-body onboard">
        ...
        {step === 3 && (<>
          ...
          <p className="step-help">Best fits for {window.SPORTS.find(s=>s.id===sport)?.label || 'your sport'} at {days} d/wk. Editable any time.</p>
...
// ---------- MAIN APP ----------
function MainApp({ profile, setProfile, theme, setTheme, onLogout, initialDays, initialTab }) {
  const baseDays = initialDays || window.INITIAL_DAYS;
  const [days, setDays] = useState(baseDays);
  const [cardioDays, setCardioDays] = useState(window.INITIAL_CARDIO_DAYS);
  const [locked, setLocked] = useState([false,false,false,false,false,false,false]);
  const [tab, setTab] = useState(initialTab || 'general');
  // splitsByType is the source of truth for "what's in a Push day", "Pull day", etc.
  // Seeded from initial days so the user's chosen onboarding template carries over.
  const [splitsByType, setSplitsByType] = useState(() =>
    window.splitsByTypeFromDays ? window.splitsByTypeFromDays(baseDays) : {}
  );
...
function Root() {
  ...
  // Onboarding result threads through here so MainApp boots on the chosen
  // template + lands the user on Dashboard rather than the default tab.
  const [bootState, setBootState] = useState({ initialDays: null, initialTab: 'general' });
