...
function computeCoverageV2(days) {
  const out = {};
  ...
  for (const day of days) {
    ...
    for (const exId of day.exIds) {
      ...
      if (!ex) continue;
      const setCount = window.setsForExercise(ex);
...
function intensityForV2(sets, target) {
  if (sets <= 0) return 0;
...
function AnatomyFront({ sets, recently, onRegion, sex='m' }) {
  const fillFor = (key) => {
...
function AnatomyBack({ sets, recently, onRegion, sex='m' }) {
  const fillFor = (key) => {
    const lvl = intensityForV2(sets[key] || 0, TARGETS_V2[key]);
    return RAMP_V2.dark[lvl];
  };
...
function Anatomy3D({ sets, view, recently, focused, onRegion, sex='m' }) {
...
Object.assign(window, {
  MUSCLE_LABELS_V2, TARGETS_V2, computeCoverageV2, exercisesForMuscle,
  AnatomyFront, AnatomyBack, Anatomy3D, intensityForV2, expandHits,
});
